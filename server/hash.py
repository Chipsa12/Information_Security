...
def main(text, type):
    ...
    if type == 'hash':
        ...
        try:
            ...
            for ch in text:
                s.append(ord(ch))
            # роль ключа в шифре играют элементы свертки, полученной на
            # предыдущей итерации
            for x in s:
                # в качестве начальнонго значение Н0 берется секретный ключ
                H = key
                # лочная ф-ция шифрования
                gost = Crypt(H, blocks)
                E_M = gost.encrypt(x)
                H = E_M ^ H
                cyphred = E_M
